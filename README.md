...
What "private" enforces — one rule: **a session can neither read nor modify another project's private content.** Automatic surfaces (briefings, prompt/pitfall injections, subagent context, predictive recall) never carry it elsewhere. Explicit reads (`cairn_recall`, `cairn_export`, `cairn_expand`, `cairn_plan`, `cairn_reminder_list`, cleanup/stats previews, the plan and briefing resources) are **session-bound** — they return private content only when the session's own working directory is that project; naming the project as an argument from elsewhere is not enough. Mutations follow readability: `cairn_forget`, `cairn_correct`, `cairn_strengthen`, `cairn_weaken`, cleanup execution, and reminder management refuse private targets from other sessions. Moving content out requires both standing *and* acknowledgment: `cairn_promote` and restore-mode `cairn_ingest` work only from a session inside the project, with `from_private: true`. Writing IN is deliberately allowed: `cairn_learn` (and restore-mode inserts of new rows) may target a private project from any session — content flowing in leaks nothing, and it keeps cross-session seeding workflows working. This is isolation for autonomous agents sharing one memory store — not access control against you: the database file is yours either way.

One caveat on session identity: it derives from the server's working directory (git remote when available, path hash otherwise). For non-git projects, launch your agent from the project root — a subdirectory or symlinked path derives a different identity and the private project's content will (safely) not be returned.
