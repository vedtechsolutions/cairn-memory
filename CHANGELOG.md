...
- Added `waykeep migrate` (Phase B2, dual-reviewed): migrates the home store from a legacy `~/.cairn` to `~/.waykeep` and writes the marker that makes the new root authoritative. The legacy store is COPIED, never moved (it stays intact as a rollback path); the database is copied with SQLite's WAL-safe online-backup API from a read-only source. The db and config migrate as one coherent unit — a divergent target `config.json` (a fork artifact) is moved aside and the legacy config applied, so the user's privacy settings never silently fail to carry. The copy is verified before the marker with `integrity_check`, `foreign_key_check`, and a per-table row-count manifest across EVERY table (not just `memories`), plus a final re-read of the legacy store that aborts if it changed at all during the copy; the db, config and directory are durably fsync'd (a real I/O/space failure aborts) and the marker published LAST via a temp-file + atomic rename; a cross-process lock serializes concurrent runs (the marker is re-checked under it); the target dir is forced owner-only (0700) and verified self-owned before it is made authoritative; a symlink at the db/config/marker path is moved aside rather than followed; and a pre-existing target db/config is moved aside to a collision-free backup rather than clobbered. It ABORTS if a daemon is still serving the store (stop it first, so no memory committed after the copy is lost). Idempotent (marker present, or no legacy store, ⇒ no-op) with a `--dry-run` preview; it tells you to restart the agent after.
...
- Added two source-hygiene ratchets: no file may gain un-named numeric literals or grow past the line limit (`tests/source-ratchets.test.ts` against pinned baselines; `scripts/source-ratchets.mjs --write` re-pins after a cleanup and refuses to pin growth unless `--force` is passed deliberately).
...
- Split the 598-line SessionEnd hook into a thin entry point, a handler and four stage modules with no observable change: `handlers/session-end-handler.ts` orders the stages; `shared/session-end/final-snapshot.ts` (the last compaction snapshot and decision mining), `session-quality.ts` (the quality signal, its label and summary), `retrospective.ts` (iteration-cost pitfalls and pattern mining) and `session-consolidation.ts` (the end-of-session merge pass). Each stage is the hook's former block moved verbatim into a function whose parameters are exactly the locals it read; `SessionQuality` is still exported from `session-end.ts`. The entry point now closes its database client on the error path too.
- Split the 641-line maintenance module into its sweeps with no public change: `maintenance-cleanup.ts` (snapshots, untouched and archived plans, a project's memories, stale projects, telemetry, the governance evidence ceiling), `staleness.ts` (zero-impact pitfalls, stale fingerprints, deleted-file references, anchor repair after renames, the staleness run) and `consolidation-runner.ts` (embedding similarity, per-cluster consolidation, guarded auto-promotion); `maintenance.ts` keeps the rate-gated `runMaintenance` and `MaintenanceResult` and re-exports every sweep, so every existing import path still works. Every declaration is an exact slice of the original, cut at its own declaration boundaries; the only textual changes are the dropped "Stale Memory Detection" section banner (its content now heads `staleness.ts`) and the reworded `maintenance.ts` module header.
- Split the 710-line worktree digest into its two readers and their shared primitives with no public change: `worktree-digest-shared.ts` (versions and the hard ceiling, the option and result shapes, the snapshot model, hashing and canonical JSON, the relevant-path glob grammar, containment, git record parsing, the deadline check), `worktree-digest-v1.ts` (the synchronous reader), `worktree-digest-v2-facts.ts` (status, index and bounded file facts) and `worktree-digest-v2.ts` (the concurrent reader); `worktree-digest.ts` re-exports the public surface. Every declaration is an exact slice of the original, cut at its own declaration boundaries; the only byte-level change is the `export` keyword on the fourteen formerly private declarations the sibling modules import.
- Split the 758-line memory tools registrar into one module per tool group with no public change: `memory-tool-deps.ts` (the shared dependency object, the reranker seam and the cache bump), `recall-tool.ts` (with the mode-adjusted limit helper), `learn-tool.ts` (with the cross-kind edge helper), `curation-tools.ts` (correct, forget, strengthen, weaken behind the private-project mutation block), `expand-tool.ts` and `cleanup-tool.ts`. `registerMemoryTools` keeps its signature and composes the five registrars, and `RerankerImpl` is still exported from `memory-tools.ts`; every tool body is an exact slice of the original (the eight registrations keep their order; the only byte-level changes are the trailing space the original left after `registerToolCompat(server,` and the `export` keyword on the moved `bumpCache` helper).
- Split the 1,229-line governance repository into focused stores with no public change: `repository-types.ts` (the contract types, bounds and typed error), `recorder-store.ts` (tool-event and gate-run inserts, mutation-sequence and retention bookkeeping, client state, Stop observation), `shadow-rows.ts` (row shapes, the bounded identifier grammar, the fault classifier and the row parsers), `shadow-snapshot-store.ts` (the one-snapshot evaluator read and rule watermarks), `shadow-verdict-store.ts` (the bounded verdict payload, sequence re-check and audit insert) and `repository-cleanup.ts` (evidence and lifecycle retention). `GovernanceRepository` keeps every method as a thin facade over those functions and re-exports every name the module exported (plus the two cleanup option types that were inline before, `GovernanceEvidenceCleanupOptions` and `GovernanceLifecycleCleanupOptions`), so no import path changes; every moved body is an exact slice of the original, dedented by the two spaces of the class body (which also trims two leading spaces from each line of the multi-line SQL strings — the same SQL, not the same bytes).
- Made each direct-node hook entry point run the same implementation as its daemon handler instead of a diverged twin: the StatusLine script and handler share one core (`hooks/shared/statusline-core.ts`: context-mode arithmetic, the three status queries, the bar format); the SubagentStop, plan-bridge and success-tracker scripts now call their handlers, owning only stdin/stdout, the database client, the tracker file lock, telemetry and (for the success tracker) the governance tee. The StatusLine counts are now all-or-nothing on a query error on both paths, where a late failure could previously print a step fragment without a memory count. The direct-node path thereby gains what the daemon path already had — SubagentStop note de-duplication, the resume cursor, per-file edit counts, patch-envelope file paths — and the success tracker's file lock still wraps the shared logic (`trackSuccess` on a caller-owned tracker, which now also records the edit when the database cannot be opened, skipping only the database-dependent steps). The direct success path also admits every edit tool (`apply_patch` included), the plan-bridge script opens its database client before plan discovery rather than only on creation, and a plan-creation failure there is reported as the script's generic `error` telemetry instead of `create-error`. The SubagentStop message thresholds are named in `LIMITS`, and the StatusLine script reads stdin through the shared reader and logs a swallowed failure at debug level.
- Replaced the duplicated helpers the audit found with one shared utility each (`src/utils/`): one text truncation (`truncate`/`truncateAscii`, with the ellipsis and every clip budget named in `constants/budgets.ts`, replacing seven private copies across the briefing, transcript, plan-parser, signal, decision and project-scanner code), one Jaccard overlap (`similarity.ts`, replacing the truth-evaluator, fingerprint and briefing query-fingerprint copies), one POSIX shell quoting (`shell.ts`, replacing two differently-spelled character classes in the CLI and the governance recorder), one JSON-object shape guard (`plain-object.ts`, replacing nine inline checks across the CLI, config loader, sync eligibility, owner RPC and four governance modules), one canonical-path resolver (`fs-paths.ts`, shared by `doctor` and the Claude health check), one tag slug (`importers/shared.ts` `slugOf`, with `LIMITS.SLUG_MAX_CHARS`), and the passwd-backed `robustHomedir` exported from `constants/paths.ts` and used for the thirteen remaining bare `homedir()` calls across eleven files (Claude/Codex config and session dirs, `~` expansion, the StatusLine state file, transcript containment), so those paths resolve through the same absolute home as the store — a relative `HOME` can no longer point them at a CWD-relative location, and the three sites with a documented failure contract keep it when no home resolves at all: `readState` stays fail-open, transcript containment fails closed, and `init`'s advisory other-client scan (now `cli/other-clients.ts`) reports none rather than failing after configuration was written. Behavior is otherwise unchanged except where noted under Fixed.
- Moved the remaining module-local tuning constants under `src/constants/`: the governance command, override, warning, shadow-evaluator and override-context bounds (`GOVERNANCE_BOUNDS`), decision sigils, secret-scanner and branch-goal bounds, the transcript head-read and cursor-read sizes, the governance briefing tier budget, the narrow-overlap fingerprint policy, the config-cache mtime granularity, the engine floor, the executable file mode, the context-free percentage bounds, the layer-norm epsilon, and the memory-tool render cache, record sanity limit and token prefix length. Values unchanged; the numeric-literal baseline dropped accordingly.
- Centralized the tuning values that lived beside the code that used them (audit): a day in milliseconds (eleven expressions across ten files), the session-cache TTLs, the decision-reflector limits, the worktree-digest and governance-rule caps, the hook-socket and relay-probe timeouts, the rollup busy timeout and daemon timings, the user-model scoring factors, the transcript truncation lengths, the Codex hook timings, and the `.claude`/`.codex` directory names now live under `src/constants/` (new `time.ts`, `codex.ts`, `memory-tool.ts`); the `/health` and `/statusline` socket routes are spelled once and reach the shell relay through the identity generator; `waykeep_cleanup` takes its action vocabulary from the declared constant.
...
- Fixed the hermetic test preload leaving the default database path unpinned: `WAYKEEP_DIR` relocates the edit trackers, the daemon socket and the error-dedup file but, by design, neither the database nor the config file, so a test that spawned a hook entry point wrote into the developer's real store. The preload now pins `WAYKEEP_DB_PATH` under the hermetic state directory unconditionally (an exported live-store path is overridden, never inherited), refuses an inherited `WAYKEEP_DIR` or `WAYKEEP_STATE_PATH` that points into the real home store, and the development docs say which override moves what.
- Fixed the load-sensitive tests that failed on a busy host and passed alone. The relay mocks in `hook-relay-status`, `relay-socket-resolution` and `governance-gate-relay` answered before consuming the request body, so the relay's second write hit EPIPE and it fell back silently with empty output (reproduced with the relay's own fallback log); they now answer after the body, as the real daemon does. The governance shadow and warn corpora ran real git on temporary worktrees under the 1 s production digest ceiling and the evaluator's 1 s budget; they now use a generous test digest deadline and a deterministic evaluator clock (timing keeps its own fault-injection and digest tests), and the digest test's own captures run under the same budget, with its ceiling assertion requiring the best of three samples under the ceiling instead of every sample. The governance watchdog test now sets the governance timer explicitly and bounds a hung daemon above only against a hang, instead of a fixed 1 s bound that also measured host scheduling.
- Fixed the `memory.md` importer tagging a sibling note's sections with a dash-edged topic (a file named `_notes.md` produced `topic:-notes`); the file-name slug now uses the same rule as every other slug and trims edge dashes, and a slug cut at the length cap no longer ends in a dash.
- Fixed the MCP server reading the StatusLine state file through its own unvalidated copy of the reader: it skipped the staleness bound, skipped the validation that rejects a forged `mode: "critical"` (which would silently suppress memory tooling), and ignored the `WAYKEEP_STATE_PATH` override. Tool registration now uses the one validated `readState()`, and a structural test pins the single-reader property.
- Fixed the session-start briefing budget using hardcoded copies of the context thresholds (50/25/10) instead of `CONTEXT_THRESHOLDS`, so a threshold retune changed the StatusLine mode but not the briefing budget.
- Fixed the plugin-launcher executable-shim test failing on GitHub runners (red CI on `main` since the marketplace work): the test shim ran a bare `exec node` under a PATH that only a system node on `/usr/bin` satisfied; it now execs the absolute node path, as real volta/pnpm shims do.
- Fixed `waykeep init` writing the Claude Code MCP server into `~/.claude/settings.json`, where Claude Code never reads it (that key is silently ignored there — the server was unreachable via `/mcp` while init reported success): init now registers the server at user scope through `claude mcp add-json` (re-pointing a moved install with `claude mcp remove` first, removing a retired `cairn` alias of this install, and only reporting one that launches another install), finds `claude` on PATH or in the usual install locations (`~/.local/bin`, Homebrew, `/usr/local/bin`, newest nvm; `WAYKEEP_CLAUDE_BIN` overrides), and when it still cannot, prints the exact command, repeats it last as an action required and says so in the summary; exits non-zero when the CLI fails or reports success without the registry showing it (a failed re-point prints the command that restores the previous entry); honors `CLAUDE_CONFIG_DIR` for both files; sweeps the inert block older versions wrote; and `--statusline-only` now also removes the user-scope registration the plugin supersedes (an entry it does not recognize as this install's is reported with the removal command, never called absent — the bin form `waykeep serve` and the plugin launcher are recognized as Waykeep servers too). `waykeep doctor` gained a `claude mcp` check that warns while the server is not registered with Claude Code (user scope or plugin) or an inert settings.json block lingers. The test suite gained hermetic overrides for `~/.claude/settings.json`, `~/.claude.json`, and the `claude` binary so no test can rewire a real Claude Code.
