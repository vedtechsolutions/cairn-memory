...
## Cairn — Primary Memory System

Cairn is your primary memory. Use it instead of file-based auto memory (MEMORY.md).
Do NOT write memories to MEMORY.md — use cairn_learn instead.
Do NOT read MEMORY.md for context — use cairn_recall instead.

### Automatic (hooks — no action needed)
- **SessionStart**: Briefing injected with plan state, pitfalls, corrections, decisions
- **StatusLine**: Context pressure written to `~/.claude/cairn-state.json` for dynamic briefing budgets
...
- **PostToolUse (ExitPlanMode)**: Plan bridge — auto-persists plan mode plans to Cairn's DB
...
- `cairn_recall(query)` — ALWAYS recall before starting work. Uses hybrid search (keyword + semantic embeddings) and enriches results with knowledge graph neighbors. The hook pre-fetches some context, but explicit recall gives you decisions and facts the hook may miss.
- `cairn_plan(get)` — check for active plan (especially after compaction)

**After EVERY significant decision:**
- `cairn_learn(kind: "decision", content)` — YOU MUST store decisions when you choose between alternatives, select an architecture, pick a dependency, or decide on an approach. Include the rationale. This is the single most important active behavior.
- `cairn_plan(decide, chose, why, permanent)` — if a plan is active, record the decision there too
...
- **Three-layer Stop-hook capture pipeline** (Layer 1a → 1b → 1c, in order):
  ...
  - **Tier-3 nudge** — when 1c returns empty (sampling unavailable, API error, or LLM found nothing), the next UserPromptSubmit emits a single-line reminder: `[CAIRN] Last turn had N decision markers but no sigil...`. Treat this as a prompt to emit sigils next time or call `cairn_learn(decision)` directly. It clears automatically after one display.

**During work:**
- `cairn_plan(step, step_id, status)` — mark steps done/in_progress/blocked
- `cairn_plan(note, note)` — progress notes (max 150 chars)
- `cairn_strengthen(id)` — memory proved useful
- `cairn_weaken(id)` — memory was wrong or unhelpful

**When learning happens:**
- `cairn_learn(kind: "pitfall", content)` — mistake encountered: what + why + fix
- `cairn_learn(kind: "correction", content)` — user corrects you: one-sentence lesson
- `cairn_learn(kind: "fact", content)` — stable knowledge, user preferences
- `cairn_learn(kind: "user_profile", content)` — user's role, expertise, preferences (always global scope)
- `cairn_learn(kind: "reference", content, tags: ["linear"])` — pointers to external systems (tags auto-prefixed with ref:)
- Optional structured context: `why: "reason"`, `how_to_apply: "guidance"` on any cairn_learn call
- Corrections and user_profile default global scope. All else defaults project-scoped.

**Task lifecycle:**
- Plan mode plans auto-persist — ExitPlanMode hook bridges to Cairn automatically
- `cairn_plan(create, name, steps)` — create plan manually when not using plan mode
- `cairn_plan(complete)` — mark plan done, graduates permanent decisions to memory
- `cairn_remind(trigger, action)` — "when I see X, remind me to Y"

**Maintenance:**
- `cairn_export(project, kind?, min_confidence?)` — review stored knowledge
- `cairn_promote(id)` — promote project memory to global (pitfall/decision, conf >= 0.7)
- `cairn_correct(id, action: "update"|"invalidate", new_content?)` — fix/remove memory
- `cairn_reminder_list(project?)` — list active reminders
- `cairn_reminder_delete(id, permanent?)` — deactivate or delete a reminder

### Pre-Implementation Workflow (mandatory before writing ANY code)
1. `cairn_recall(query)` — recall prior decisions, pitfalls, and mistakes for this task
...
- If briefing shows [interrupted], call cairn_plan(get) immediately
