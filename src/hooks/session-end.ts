...
import { parseTranscript, extractWinningPattern } from './shared/transcript-parser.js';
...
try {
  ...
  try {
    const snapshot = parseTranscript(input.transcript_path);
  ...
  try {
    ...
    if (overBudget.length === 0 && quality.label === 'smooth' && quality.stepsCompleted > 0) {
      try {
        const retroSnapshot = parseTranscript(input.transcript_path);
